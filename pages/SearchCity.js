    ...
    return (
      <Container>
        <Header searchBar rounded
          ...
          <TouchableOpacity onPress={() => this.props.navigation.goBack()} style={{ width: 20 }}>
            <Icon active name='ios-arrow-back' style={{ color: '#fff', top: 15, }} />
          </TouchableOpacity>
          <Item>
            <Icon name="search" />
            <Input
            ...
            <Icon name={this.state.searchQuery != '' ? 'close' : 'home'} onPress={() => this.resetSearchQuery()} />
          </Item>
        </Header>
        <Content>
          {/* search result */}
            ...
            {this.state.data.map((e, i) => {
              return (
                <ListItem avatar key={i}>
                  <TouchableOpacity style={{ flexDirection: 'row' }} onPress={() => this.presed(e.kota)}>
                    <Body>
                      <Text>{e.kota}</Text>
                    </Body>
                    <Right>
