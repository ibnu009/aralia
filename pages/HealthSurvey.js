...
export default class SurveyScreen extends Component {
    ...
    onSurveyFinished(answers) {
            ...
            for(const ques of this.state.listQuesioner) {
                ...
                for(const answerQuest of data) {
                    if(answerQuest.questionId == ques.id_quesioner) {
                        if(answerQuest.value.value) { // value with options
        ...
        this.saveSurvey(id_employee, curDate, id_quesioners, sequences, questions, types, id_opts, opt_names, descs);
    ...
    getSurveyElement() {
        if(this.state.survey.length > 0){
            ...
            return (
                <Text style={{textAlignVertical: "center",textAlign: "center",}}>Sedang memuat data..</Text>
            )
        }
    }
    ...
    getSurvey(){
        ...
        .then(res => {
            if(res.status == "OK") {
                ...
                if(res.list_quesioner.length > 0) {
                    for(const item of res.list_quesioner) {
                        if(item.list_opt.length > 0) { // combobox
                            ...
                            quesioners.push(this.itemGroup(item.question, item.id_quesioner, item_list));
                        } else {
                            quesioners.push(this.itemInput(item.question, item.id_quesioner, "Silahkan di isi.."));
                        }

                    }
                }
                quesioners.push(this.itemInfo("Anda telah mengisi quesioner hari ini. Terimakasih"));
            ...
            //this.setState({ showLoading: false });
            Alert.alert('Info', `Gagal memuat data`, [
                { text: 'Close' },
            ]);
            this.props.navigation.navigate('CheckInOut');
        });
    ...
    saveSurvey(id_employee, curDate, id_quesioners, sequences, questions, types, id_opts, opt_names, descs){
        ...
        fetch(uri, requestOptions)
          ...
          .then(result => {
              if(result.status == "OK") {
                ...
                Alert.alert('Info', `Gagal simpan data`, [
                    { text: 'Close' },
                ]);
              }
    ...
    itemMultipleGroup(question, questionId, mxMultiSelect = 3, mnMultiSelect = 2, optionData=[]) {
        /**
         *      Data options list
         *      [{
