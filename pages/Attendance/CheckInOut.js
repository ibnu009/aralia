...
import { REST_URL, REST_PGD, HEADERS_CONFIG } from 'aralia/AppConfig';
import {
  StyleSheet,
  TouchableOpacity,
  View,
...
class CheckInOut extends Component {
  ...
  componentDidMount() {
      ...
      if((quesionerData['checkQuesioner'] < 1) && (quesionerData['isQuesioner'] == "1")){
      ...
      .then(res => {
        ...
        if(this.checkAttendanceModal(userData['level'])){
              ...
              if(Array.isArray(res.shift)) {
                if(res.shift.length > 0){
                        ...
                        { 
                          ...
                          onPress: () => {
                            ...
                            res.shift.forEach(function(data){
                              ...
                              shift_data.push({label : `${data.shift} - ${data.time_in} s/d ${data.time_out} `, value: data.id_tr_shift})
                              i++
                            })
    
                            this.setState({allow_absen : 'Y', itemShift : shift_data, id_tr_shift : value_selected, objCheck : res, BU_DISPLAY : 'flex', displayShift: (res.shift.length > 1 ? 'flex' : 'none'),}, () => {
                              if((res.type_attendance == "A" && res.flag == "in") || (res.flag == "in")){
                                this.setState({
                                  A_IN : false
                                })
                              }
                              ...
                              if((res.type_attendance == "BU" && res.flag == "in") || (res.flag == "in")){
                                this.setState({
                                  BU_IN : false
                                })
                              }
                  ...
                  if(res.status == "success"){
                      ...
                      this.setState({itemShift : shift_data, id_tr_shift : value_selected, objCheck : res, BU_DISPLAY : 'flex', displayShift: (res.shift.length > 1 ? 'flex' : 'none'),}, () => {
                        if((res.type_attendance == "A" && res.flag == "in") || (res.flag == "in")){
                          this.setState({
                            A_IN : false
                          })
                    ...
                    Alert.alert(
                      ...
                      [
                        ...
                        { 
                          ...
                          onPress: () => {
                            var shift_data = []
                            this.setState({itemShift : shift_data, id_tr_shift : "", BU_DISPLAY : 'flex', objCheck : res, displayShift: 'none' ,}, () => {
                              if((res.type_attendance == "A" && res.flag == "in") || (res.flag == "in")){
                                this.setState({
                                  A_IN : false
                                })
          ...
          if(res.status == "error") {
              ...
              Alert.alert(
                ...
                [
                  ...
                  },
                  {
                    text: 'Ya', 
                    onPress: () => {
                      this.setState({ allow_absen : 'Y', itemShift : [], id_tr_shift : '', objCheck : res, displayShift: 'none'});
                    }
                  }
                ],
                { cancelable: false }
              ...
              this.setState({itemShift : [], id_tr_shift : '', objCheck : res, displayShift: 'none'});
          }
        }
      })
      .catch(err => {
          ...
          if(res.status == "success"){
            ...
            if(Array.isArray(res.shift)) {
              ...
              if(res.shift.length > 0){
                ...
                this.setState({itemShift : shift_data, id_tr_shift : value_selected, objCheck : res, displayShift: (res.shift.length > 1 ? 'flex' : 'none') ,}, () => {
                  if((res.type_attendance == "A" && res.flag == "in") || (res.flag == "in")){
                    this.setState({
                      A_IN : false
                    })
  ...
  getLocationName = () => {
    this.getLatLongPosition
      .then(() => {
        ...
        .then(res => {
          if(res["data"]){
            this.setState({ id_geo:res["data"]["id"], is_location : true, location: `Anda berada di ${res["data"]["area_name"]}` });
          } else {
            this.setState({ is_location : false, location: `Anda tidak berada didalam area.` });
          }
        })
        .catch(err => {
          // console.log(err);
          this.setState({ is_location : false, location: `Gagal mendeteksi area.` });
        });
      })
      .catch(err => {
        if (err.alert) {
  ...
  takePicture = async () => {
    ...
    var userData = UserStore.getUserData();
    if(this.state.attendance_type == "" && this.checkAttendanceModal(userData['level']) || (this.state.attendance_type == "" && this.state.objCheck.shift != "") ){
        this.setState({modalShow: true}) 
    } else {
        ...
        if((this.state.is_location == true) || (isGeofence == "0")) {
          ...
          Alert.alert('Info', 'Tidak bisa melakukan absen, anda harus berada di sekitar area kantor.');
        }
    }
  };
  ...
  doAttendance = () => {
    ...
    if(this.state.objCheck != null) {
      ...
      if (is_submit_attendance) {
        ...
        formData.append(type_date, Moment(new Date()).format('YYYY-MM-DD HH:mm:ss'));
        ...
        .catch(err => { 
          console.log(err);
          Toast.show(`Error when doing saving attendance :\n ${err}`, Toast.SHORT);
          this.setState({ showLoading: false });
          this.props.navigation.goBack();
        });
      } else {
          ...
          >
            <View style={styles.centeredView}>
              <View style={styles.modalView}>
              {
                  (
                    () => {
                      if((this.state.objCheck != null) && (this.state.itemShift.length > 0) ){
                        return (
                          <View  style={{width:160, top: 0, marginBottom : 30, display: this.state.displayShift}}>
                            <DropDownPicker 
                ...
                <View style={styleLegend.fieldSet}>
                    ...
                    <TouchableHighlight
                     disabled={this.state.A_IN}
                      style={{ ...styles.openButton, marginTop:20, backgroundColor: this.state.A_IN ? '#f1f1f1' : '#008000', width: 60, right: 0, margin : 4 }}
                      onPress={() => {
                        this.setState({modalShow:false, attendance_type: "A", attendance_flag: "in"})
                      }}
                    >
                    ...
                    style={{ ...styles.openButton, marginTop:20, backgroundColor: this.state.A_OUT ? '#f1f1f1' : '#008000', width: 60, left:0, margin : 4 }}
                    onPress={() => {
                      this.setState({modalShow:false, attendance_type: "A", attendance_flag: "out"})
                    }}
                    >
                ...
                />
                <View style={{...styleLegend.fieldSet, ...{display: this.state.BU_DISPLAY}}}>
                    <Text style={styleLegend.legend}> BACKUP </Text>
                    <TouchableHighlight
                      disabled={this.state.BU_IN}
                      style={{ ...styles.openButton, marginTop:20, backgroundColor: this.state.BU_IN ? '#f1f1f1' : '#008000', width: 60, left:0, margin:4 }}
                      onPress={() => {
                        this.setState({modalShow:false, attendance_type: "BU", attendance_flag: "in"})
                      }}
                    >
                      <Text style={styles.textStyle}>IN</Text>
                    </TouchableHighlight>
                    ...
                    <TouchableHighlight
                      ...
                      style={{ ...styles.openButton, marginTop:20, backgroundColor: this.state.BU_OUT? '#f1f1f1' : '#008000', width: 60, right: 0, margin: 4 }}
                      onPress={() => {
                        this.setState({modalShow:false, attendance_type: "BU", attendance_flag: "out"})
                      }}
                    >
                      <Text style={styles.textStyle}>OUT</Text>
                    </TouchableHighlight>
