...
import { MONTH_ITEMS_OPTION } from "aralia/AppConfig";
import {
  StyleSheet,
  ActivityIndicator,
  Dimensions,
...
class AttendaceList extends Component {
  ...
  componentDidMount() {
    try {
      const granted = PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE,
        {
          ...
          message:
            "Aralia App needs access to your folder ",
    ...
    if ('' != UserStore.getUserData()){
      ...
      actions.getAbsensiData(nip, this.state.selectedYear, this.state.selectedMonth);
    }
  }
  ...
  submitButton = () => {
    ...
    actions.setMonth(this.state.selectedMonth);
    actions.setYear(this.state.selectedYear);
    actions.getAbsensiData(this.state.nip, this.state.selectedYear, this.state.selectedMonth);
  };
